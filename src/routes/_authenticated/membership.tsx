...
import { getMyMembership, submitMembership } from "@/lib/membership.functions";

export const Route = createFileRoute("/_authenticated/membership")({
  head: () => ({ meta: [{ title: "Park Avenue HOA Membership — Ez-ARC" }] }),
...
function MembershipPage() {
  ...
  return (
    <div className="mx-auto max-w-2xl">
      ...
      {hoa?.description && (
        <p className="mt-2 text-muted-foreground">{hoa.description}</p>
      )}

      {membership && !editing && (
        <div className="mt-6">
          <StatusBanner status={membership.status} reason={membership.rejection_reason} />
  ...
  if (status === "approved")
    return (
      <Banner
        ...
        icon={CheckCircle2}
        title="Approved"
        body="You're a member of Park Avenue HOA. You can now submit ARC applications."
      />
    );
  return (
    <Banner
